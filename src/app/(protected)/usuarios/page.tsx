...
const ROLES: Role[] = ['admin', 'governanta', 'subgovernanta', 'recepcion', 'camarera', 'camarera_guardia'];
...
export default function UsuariosPage() {
  ...
  async function eliminar(u: AppUser) {
    if (!confirm(`¿ELIMINAR del todo a ${u.displayName}? Esta acción es permanente y borra su acceso y sus datos de usuario.`)) return;
    if (!confirm('Confirma de nuevo: esto no se puede deshacer.')) return;
    try {
      await httpsCallable(fns(), 'eliminarUsuario')({ uid: u.uid });
      setMsg(`${u.displayName} eliminado.`);
      cargar();
    } catch (e: any) {
              ...
              <tr key={u.uid} className="border-t border-gray-100">
                ...
                <td className="px-3 py-2">
                  <div className="flex flex-wrap justify-end gap-1.5">
                    ...
                    <button onClick={() => eliminar(u)} className="rounded-md border border-red-300 px-2 py-1 text-xs text-red-700 hover:bg-red-50">Eliminar</button>
                  </div>
                </td>
              </tr>
            ))}
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4" onClick={onClose}>
      <div className="w-full max-w-sm rounded-xl bg-white p-5 shadow-lg" onClick={(e) => e.stopPropagation()}>
        ...
        <select value={role} onChange={(e) => setRole(e.target.value as Role)}
          ...
          {(['admin', 'governanta', 'subgovernanta', 'recepcion', 'camarera', 'camarera_guardia'] as Role[]).map((r) => (
